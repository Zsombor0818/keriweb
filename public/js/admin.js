...
window.openNewsModal = function(id) {
    ...
    <div class="field">
      ...
      <div class="quill-wrap">
        <div id="mNewsQuillToolbar">
          ...
          <span class="ql-formats">
            <button class="ql-image-picker" title="Kép beillesztése a médiatárból" onclick="openImagePicker()">🖼️</button>
            <label class="ql-image-upload" title="Kép feltöltése és beillesztése">
              ⬆️<input type="file" id="quillDirectUpload" accept="image/*" style="display:none" onchange="quillDirectUploadFn(this)">
            </label>
          </span>
          <span class="ql-formats">
        ...
        <div id="mNewsBody"></div>
      </div>
    </div>
    <div class="field"><label><input type="checkbox" id="mNewsPublished" ${(!n || n.published) ? 'checked' : ''}> &nbsp;Közzétéve</label></div>

    <!-- Képválasztó panel (rejtett) -->
    <div id="imgPickerPanel" class="img-picker-panel" style="display:none">
      <div class="ipp-head">
        <strong>Médiatár – válassz képet</strong>
        <div class="ipp-head-right">
          <label class="btn-primary btn-sm" style="cursor:pointer">
            ⬆️ Új kép feltöltése
            <input type="file" accept="image/*" style="display:none" onchange="uploadAndPickImage(this)">
          </label>
          <button class="btn-ghost btn-sm" onclick="closeImagePicker()">✕ Bezárás</button>
        </div>
      </div>
      <div class="ipp-search">
        <input type="text" placeholder="Keresés fájlnév alapján…" oninput="filterPickerImages(this.value)">
      </div>
      <div class="ipp-grid" id="ippGrid">
        <div class="ipp-loading">Képek betöltése…</div>
      </div>
    </div>`;
  ...
  quillEditor = new Quill('#mNewsBody', {
    theme: 'snow',
    modules: { toolbar: { container: '#mNewsQuillToolbar' } },
  });
...
async function refreshPickerImages() {
  const grid = document.getElementById('ippGrid');
  if (!grid) return;
  try {
    const all = await api('/api/media');
    _pickerImages = all.filter(f => f.mimetype && f.mimetype.startsWith('image/'));
    renderPickerGrid(_pickerImages);
  } catch (e) {
    grid.innerHTML = `<div class="ipp-loading" style="color:var(--danger)">Hiba: ${esc(e.message)}</div>`;
  }
}
...
function renderPickerGrid(images) {
  ...
  if (!grid) return;
  if (!images.length) {
    grid.innerHTML = '<div class="ipp-loading">Nincs még feltöltött kép. Tölts fel egyet a fenti gombbal!</div>';
    return;
  }
  grid.innerHTML = images.map(f => `
    <div class="ipp-item" onclick="insertImageFromPicker('/uploads/${esc(f.filename)}', '${esc(f.original_name||f.filename)}')" title="${esc(f.original_name||f.filename)}">
      <div class="ipp-thumb">
        <img src="/uploads/${esc(f.filename)}" alt="${esc(f.original_name||'')}">
      </div>
      <div class="ipp-name">${esc(f.original_name||f.filename)}</div>
      <div class="ipp-size">${fmtSize(f.size)}</div>
    </div>`).join('');
}

window.filterPickerImages = function(q) {
  const filtered = _pickerImages.filter(f =>
    (f.original_name||f.filename).toLowerCase().includes(q.toLowerCase())
  );
  renderPickerGrid(filtered);
};

window.insertImageFromPicker = function(url, alt) {
  if (!quillEditor) return;
  const range = quillEditor.getSelection(true);
  quillEditor.insertEmbed(range.index, 'image', window.location.origin + url);
...
window.quillDirectUploadFn = async function(input) {
  ...
  try {
    ...
    input.value = '';
    insertImageFromPicker(result.url, file.name);
    toast('Kép feltöltve és beillesztve ✓');
  } catch (e) { toast(e.message, 'error'); }
};
