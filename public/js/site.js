...
async function loadSettings() {
  ...
  if (fc)
    fc.innerHTML = `${s.address || ""}<br><span style="color:var(--st)">${s.phone1 || ""} | ${s.phone2 || ""}</span>`;
  ...
  if (kCim)
    kCim.innerHTML = `<strong>Szent István Katolikus Technikum és Gimnázium</strong><br>${s.address || ""}`;
  if (kTel)
    kTel.innerHTML = `<a href="tel:${s.phone1}" style="color:var(--cr);font-weight:600">${s.phone1 || ""}</a><br><a href="tel:${s.phone2}" style="color:var(--cr);font-weight:600">${s.phone2 || ""}</a>`;
  if (kEmail)
    kEmail.innerHTML = `<a href="mailto:${s.email}" style="color:var(--cr);font-weight:600">${s.email || ""}</a>`;
  ...
  if (socEl) {
    socEl.innerHTML = "";
    if (s.facebook_url)
      socEl.innerHTML += `<a href="${s.facebook_url}"  target="_blank" title="Facebook">f</a>`;
    if (s.instagram_url)
      socEl.innerHTML += `<a href="${s.instagram_url}" target="_blank" title="Instagram">in</a>`;
    if (s.youtube_url)
      socEl.innerHTML += `<a href="${s.youtube_url}"   target="_blank" title="YouTube">▶</a>`;
  }
  if (footSoc) {
    footSoc.innerHTML = "";
    if (s.facebook_url)
      footSoc.innerHTML += `<a href="${s.facebook_url}"  target="_blank">Facebook</a>`;
    if (s.instagram_url)
      footSoc.innerHTML += `<a href="${s.instagram_url}" target="_blank">Instagram</a>`;
    if (s.youtube_url)
      footSoc.innerHTML += `<a href="${s.youtube_url}"   target="_blank">YouTube</a>`;
  }
}
...
function fmtDate(dt) {
  if (!dt) return "";
  return new Date(dt).toLocaleDateString("hu-HU", {
    year: "numeric",
    month: "long",
    day: "numeric",
  });
