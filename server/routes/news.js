...
router.get('/', async (req, res) => {
  try {
    ...
    res.json(await db.q('SELECT * FROM news WHERE published=1 ORDER BY created_at DESC LIMIT ?', [limit]));
...
router.get('/by-slug/:slug', async (req, res) => {
  try {
    const row = await db.q1('SELECT * FROM news WHERE slug=? AND published=1', [req.params.slug]);
...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    let base = slug, i = 1;
    while (await db.q1('SELECT id FROM news WHERE slug=?', [slug])) slug = `${base}-${i++}`;

    const ok = await db.run(
      'INSERT INTO news (slug,title,excerpt,body,tag,published) VALUES (?,?,?,?,?,?)',
      [slug, title, excerpt||'', body||'', tag||'', published === false || published === '0' ? 0 : 1]
    );
...
router.put('/:id', requireAuth, async (req, res) => {
  try {
    ...
    await db.run(
      ...
      [title, excerpt||'', body||'', tag||'', published ? 1 : 0, req.params.id]
    );
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
